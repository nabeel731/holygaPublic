  ...
  items = [
    {
      ...
      age: 25,
      job: 'Phillippines',
      images: ['images/profiles/laura.jpg'],
      distance: 25,
...
let lat=localStorage.getItem("owl_userLat");
	let lng=localStorage.getItem("owl_userLng");
	makeAjaxCall('matchableUsers?lat='+lat+"&lng="+lng).then(res=> {
       document.querySelector('.control--nope').click();
	   document.querySelector('.control--nope').click();
	 let i=1;
		res.data.forEach(user => {
		  ...
		  obj.job=user.job;
		  if(user.picture1)
			obj.images.push(user.picture1);
		  if(user.picture2)
			obj.images.push(user.picture2);
		  if(user.picture3)
			obj.images.push(user.picture3);
		  if(user.picture4)
			obj.images.push(user.picture4);
		  obj.distance=user.distance;
		  obj.description=user.bio;
		  items[i]=obj;
		  //console.log(obj);
		  i++;
        });
